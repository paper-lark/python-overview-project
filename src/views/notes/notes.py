...
class NotesView(View[NotesViewProps]):
    ...
    def _onHeaderClick(self, id):
        def _onClick(self, event):
            print('click')
            note = self.props.notesDict[id]
            self.currentNote.update_props(NoteViewProps(id, note.title, note.text, note.creationTime, note.lastChangeTime))
        return _onClick
    ...
    def _update(self):
        ...
        for i, kv in enumerate(reversed(self.props.notesDict.items())):
            ...
            self._notesHeaders.append(NoteHeader(self._notesList.container))
            self._notesHeaders[-1].grid(column = 0, row = i+1, sticky="NEWS")
            self._notesHeaders[-1].bind("<Button-1>", self._onHeaderClick(id))
            self._notesHeaders[-1].update_props(NoteViewProps(note.id, note.title, note.text, note.creationTime, note.lastChangeTime))
